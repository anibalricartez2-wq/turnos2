...
class Agente:
    def __init__(self, nombre, lim):
        ...
        self.horas = 0
        self.conteo = {'M': 0, 'T': 0}
        self.bloqueos = set() # Nueva lista de bloqueos
    ...
    def configurar(self, d_m, d_t, p_m, p_t, bloq):
        ...
        # Procesar bloqueos: convertir string "5, 12" en set de números
        if bloq:
...
if st.sidebar.button("📊 Calcular Turnos"):
    ...
    for d in range(1, dias_tot + 1):
        f = date(anio, mes, d)
        for t in ['M', 'T']:
            # Lógica: Filtra agente si horas al límite, día bloqueado o fuera de disponibilidad
