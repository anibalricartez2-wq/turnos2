...
if st.sidebar.button("📊 Calcular Turnos"):
    ...
    for d in range(1, dias_tot + 1):
        f = date(anio, mes, d)
        for t in ['M', 'T']:
            # Solo los que pueden trabajar: límite de horas + no bloqueado + día disponible
            ...
            if cands:
                # Orden de prioridad: 1. Preferencia, 2. MENOR cantidad total de turnos (Equidad), 3. Horas
                cands.sort(key=lambda x: (
                    0 if (t=='M' and d in x.pref_m) or (t=='T' and d in x.pref_t) else 1,
                    x.conteo['M'] + x.conteo['T'],
                    x.horas
                ))
                el = cands[0]
...
if st.session_state.get("calculado"):
    ...
    st.download_button("📥 Descargar PDF Completo", data=generar_pdf(st.session_state.grilla, st.session_state.resumen, mes, anio), file_name="cronograma.pdf", mime="application/pdf")
